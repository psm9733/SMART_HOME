...
void setup()
{
  ...
  pinMode(IN1, OUTPUT);
  pinMode(IN2, OUTPUT);
  pinMode(Ilumination_Digital_PIN, INPUT); // Ilumination_Digital_PIN 으로 데이터를 값을 받음
  Serial.begin(9600); // 시리얼통신을 시작합니다.
}

void loop() {
  if (digitalRead(Ilumination_Digital_PIN) == HIGH) {
    Serial.println("CDS off");  // 빛이차단되면 출력됨
  ...
  int Ilumination_Data = analogRead(Ilumination_Analog_PIN); // Ilumination_Analog_PIN에서 나오는 아날로그 신호를 변수에 대입. 
